    ...
    <div class="grid row row-cols-1 row-cols-md-3 g-4">
        ...
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ?>
                <div class="card">
                    <img src="<?= BASE_URL ?>/assets/images/<?= $row['ImagenDelLugar'] ?>" alt="<?= $row['Nombre'] ?>" width="300">
                    <h3><?= $row['Nombre'] ?></h3>
                    <p><strong>Categoría:</strong> <?= $row['CategoriaNombre'] ?></p>
                    ...
                    <p><strong>Edad Recomendada:</strong> <?= $row['EdadRecomendada'] ?>+</p>
                    <p><a href="<?= BASE_URL ?>/pages/detalle_destino.php?id=<?= $row['IdLugar'] ?>" class="btn">Ver más</a></p>
                </div>
                <?php
            }
