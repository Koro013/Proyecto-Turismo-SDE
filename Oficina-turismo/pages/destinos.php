...
// --- Cargar categorías desde la BD para el select ---
$categorias = [];
try {
    $catStmt = $pdo->query("SELECT * FROM categorias");
    $categorias = $catStmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $errorCategorias = "Error cargando categorías: {$e->getMessage()}";
}
...
<body class="bg-body bebas-neue-regular">
  ...
  <main class="container pt-5">
    <section class="form-check-inline me-0">
      ...
      <!-- Botón de Filtros (acordeón) -->
      <p class="gap-1">
        <a class="btn btn-primary mt-3 mb-2 ps-4 fs-4 container collapsed" data-bs-toggle="collapse"
          href="#listaCategoria" role="button" aria-expanded="false" aria-controls="listaCategoria"
          style="letter-spacing: 0.75vw">
          Filtros
        </a>
      </p>
      <!-- Filtros (categorías) dinámico dentro del acordeón -->
      <form class="card card-body fs-4 collapse mb-4" id="listaCategoria" method="GET" action="" target="_self">
        <h1>Categorías</h1>
        <?php if (isset($errorCategorias)): ?>
          <div class="alert alert-danger"><?= $errorCategorias ?></div>
        <?php endif; ?>
        <?php foreach ($categorias as $cat): ?>
          <div class="form-check checkbox-personalizado form-check-inline">
            <input class="form-check-input"
              ...
              id="cat<?= $cat['IdCategoria'] ?>"
              <?= $categoriaSeleccionada == $cat['IdCategoria'] ? 'checked' : '' ?>
            >
        ...
        <?php endforeach; ?>
        <div class="form-check checkbox-personalizado form-check-inline">
          <input class="form-check-input" type="radio" name="categoria" value="" id="cat0" <?= $categoriaSeleccionada == '' ? 'checked' : '' ?>>
          <label class="form-check-label" for="cat0">Todas</label>
        </div>
        <div class="form-check-inline mt-3 text-center">
          <input class="btn btn-primary boton-envio fs-4 ps-3 pt-1 pe-3 pb-1" type="submit" value="Filtrar">
        </div>
      </form>
    </section>

    <!-- GRID DINÁMICO DE DESTINOS -->
      ...
      try {
          ...
          while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
              ...
              <div class="card btn btn-light col-lg-3 p-0 me-lg-3 mb-lg-3 mb-3 col-md-12 tarjeta-destinos">
                <img src="/Oficina-turismo/assets/images/<?= htmlspecialchars($row['ImagenDelLugar']) ?>"
                     class="card-img-top tarjeta-imagen"
                     alt="<?= htmlspecialchars($row['Nombre']) ?>">
                <div class="card-body">
                  <p class="card-text fs-3"><?= htmlspecialchars($row['Nombre']) ?></p>
                  <p><strong>Categoría:</strong> <?= htmlspecialchars($row['CategoriaNombre']) ?></p>
                  <p><strong>Ubicación:</strong> <?= htmlspecialchars($row['Ubicacion']) ?></p>
                  <p><strong>Horario:</strong> <?= htmlspecialchars($row['Horario']) ?></p>
                  <p><strong>Costo:</strong> $<?= number_format($row['CostoDeVisita'], 2) ?></p>
                  <p><strong>Edad Recomendada:</strong> <?= htmlspecialchars($row['EdadRecomendada']) ?>+</p>
                  <a href="/Oficina-turismo/pages/detalle_destino.php?id=<?= $row['IdLugar'] ?>" class="btn btn-primary mt-2">Ver más</a>
                </div>
              </div>
              <?php
          }
          if (!$hayResultados) {
              echo "<div class='alert alert-info fs-4 mt-4'>No se encontraron destinos para esta categoría.</div>";
          }
      } catch (PDOException $e) {
          echo "<div class='alert alert-danger'>Error al consultar destinos: {$e->getMessage()}</div>";
      }
      ?>
    </section>
  </main>
